...
if [ -d "D:/Gava/Arduino" ]; then
	ARDUINO_PATH=D:/Gava/Arduino
	ARDUINO_VERSION=106
...
if [ $BUILD_TARGET = "win32" ]; then
	...
	#extract PortablePython_${WIN_PORTABLE_PY_VERSION}.exe \$INSTDIR/App
	#extract PortablePython_${WIN_PORTABLE_PY_VERSION}.exe \$INSTDIR/Lib/site-packages
	extract PortablePython_${WIN_PORTABLE_PY_VERSION}.exe
...
#	mv \$INSTDIR/App/* ${TARGET_DIR}/python
#	mv \$INSTDIR/App/Lib/site-packages/wx* ${TARGET_DIR}/python/Lib/site-packages/
#	mv App/Lib/site-packages/ ${TARGET_DIR}/python/Lib/site-packages/
	mv App/* ${TARGET_DIR}/python
	echo "Python copiado"
	mv PURELIB/serial ${TARGET_DIR}/python/Lib
	echo "Python serial copiado"
	...
	echo "Make do curaEngine terminou"
    if [ $? != 0 ]; then echo "Failed to build CuraEngine"; exit 1; fi
fi
echo "Make do curaEngine terminou"
...
#	cp /usr/lib/gcc/i686-w64-mingw32/4.8/libgcc_s_sjlj-1.dll $TARGET_DIR
   cp C:/MinGW/bin/libgcc_s_dw2-1.dll $TARGET_DIR
#   cp /usr/i686-w64-mingw32/lib/libwinpthread-1.dll $TARGET_DIR
   cp C:/MinGW/bin/libstdc++-6.dll $TARGET_DIR
fi
...
if (( ${ARCHIVE_FOR_DISTRIBUTION} )); then
	if [ $BUILD_TARGET = "win32" ]; then
		...
		if [ ! -z `which wine` ]; then
			...
			mv scripts/win32/Vanilla_${BUILD_NAME}.exe ./
		fi
